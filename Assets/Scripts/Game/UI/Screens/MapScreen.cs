...
{
    /// <summary>노드 맵(§12): 보스 전 최대 3회 이동, 이벤트는 그중 1회. 파티 상태·재화·원석 제작도 여기서 확인한다.</summary>
    public static class MapScreen
    {
        public static void Build(RectTransform root, GameManager gm)
        {
            ...
            var nodeRow = UIFactory.HGroup(layout, 12);
            UIFactory.FixedHeight(nodeRow, 60);
            foreach (var n in run.NodeOptions())
                UIFactory.Button(nodeRow, NodeLabel(n), () => gm.EnterNode(n), bg: n == NodeType.Boss ? Theme.Danger : Theme.PanelLight, fontSize: 22);

            var gemRow = UIFactory.HGroup(layout, 10);
            UIFactory.FixedHeight(gemRow, 44);
